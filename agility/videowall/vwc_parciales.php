        ...
        <div data-options="region:'north'" class="vwc_top" style="height:10%;padding:5px"> <!-- CABECERA -->
            <div style="display:inline-block;width=100%;padding:0px" class="vwc_header">
                ...
                <span id="vwcp_header-partialscores" style="text-align:right">
                    ...
                    <span id="vwc_header-NombreManga">&nbsp;</span> - 
                    <span id="vwc_header-ModeString">&nbsp;</span>
                </span>
            </div>
        </div>
        <div data-options="region:'west'" style="width:38%;"> <!-- LLAMADA A PISTA -->
...
<script type="text/javascript">
    ...
    $('#cronometro').Chrono( {
        ...
        onUpdate: function(elapsed,running,paused) {
            ...
            if (!running && !paused) return true; // do not update on stop
    ...
    var eventHandler= {
        ...
        'close': function (event, time) { // no more dogs in tanda
            vw_updateWorkingData(event,function(e,d){
                vwcp_updateLlamada({'Dog':-1},d); // seek at end of list
            });
        },
        ...
        'stop': function (event, time) {      // stop crono manual
            ...
            myCounter.stop();
            crm.Chrono('stop', time);
            vwcp_displayPuesto(true,crm.Chrono('getValue')/1000);
        },
