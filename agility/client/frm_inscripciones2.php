...
<div id="inscripciones-info" style="width:100%">
	...
	<div id="inscripciones-infolayout" class="easyui-layout" style="height:150px">
		...
		<div data-options="region:'west',title:'<?php _e('Datos de la Prueba');?>',split:true,collapsed:false" 
			...
			<p>
			<label for="Nombre" style="font-weight:bold"><?php _e('Denominaci&oacute;n');?>:</label>
			<input id="inscripciones-pnombre" type="text" name="Nombre" disabled="disabled" size="19"/>
			</p>
			<p>
			<label for="Club" style="font-weight:bold"><?php _e('Club Organizador');?>:</label>
			<input id="inscripciones-pclub" type="text" name="NombreClub" disabled="disabled" size="15"/>
			</p>
			<p>
			<label for="Observaciones" style="font-weight:bold"><?php _e('Observaciones');?>:</label>
		...
		<div data-options="region:'center',title:'<?php _e('Lista de jornadas de la prueba');?>'" style="width:70%">
...
<div id="inscripciones-toolbar" style="width:100%;display:inline-block">
   	<span style="float:left;padding:5px"> <!-- estos elementos deben estar alineados a la izquierda -->
   		<a id="inscripciones-newBtn" href="#" class="easyui-linkbutton"
   			...
   			onclick="newInscripcion($('#inscripciones-datagrid','#inscripciones-datagrid-search').val())"><?php _e('Nuevo');?></a>
   		<a id="inscripciones-editBtn" href="#" class="easyui-linkbutton"
   			data-options="iconCls:'icon-edit'"
   			onclick="editInscripcion('#inscripciones-datagrid')"><?php _e('Editar');?></a>
   		<a id="inscripciones-delBtn" href="#" class="easyui-linkbutton"
   			data-options="iconCls:'icon-trash'"
   			onclick="deleteInscripcion('#inscripciones-datagrid')"><?php _e('Borrar');?></a>
   	...
   	<span style="float:right;padding:5px"> 	<!-- estos elementos deben estar alineados a la derecha -->
    	<a id="inscripciones-reorderBtn" href="#" class="easyui-linkbutton"
    		...
    		onclick="reorderInscripciones(workingData.prueba)"><?php _e('Reordenar');?></a>
...
$('#inscripciones-info').panel({
	title:'<?php _e('Informaci&oacute;n de la prueba');?>',
	border:true,
	closable:true,
	closed:false,
	collapsible:true,
...
$('#inscripciones-jornadas').datagrid({
	...
	method: 'get',
	loadMsg: '<?php _e('Actualizando datos de las jornadas...');?>',
    columns:[[
...
$('#inscripciones-datagrid').datagrid({
	title: '<?php _e('Listado de inscritos en la prueba');?>',
	// propiedades del panel asociado
	fit: true,
	border: false,
	closable: false,
	...
	method: 'get',
	loadMsg: '<?php _e('Actualizando datos de inscripciones....');?>',
    ...
    columns: [[
    	...
    	{ field:'NombreEquipo',	width:10, sortable:true, align: 'right',	title: '<?php _e('Equipo');?>' },
...
addTooltip($('#inscripciones-delBtn').linkbutton(),"<?php _e('Eliminar la inscripción seleccionada de la BBDD');?>");
addTooltip($('#inscripciones-reorderBtn').linkbutton(),"<?php _e('Reasignar dorsales por orden de Club,Categoria,Grado, y Nombre');?>");
addTooltip($('#inscripciones-teamBtn').linkbutton(),"<?php _e('Abrir la ventana de gesti&oacute;n de equipos de esta prueba');?>");
addTooltip($('#inscripciones-printBtn').linkbutton(),"<?php _e('Imprimir la lista de inscritos en la prueba');?>");
addTooltip($('#inscripciones-reloadBtn').linkbutton(),"<?php _e('Borrar la casilla de b&uacute;squeda<br/>Actualizar la lista de inscripciones para la prueba');?>");
addTooltip($('#inscripciones-datagrid-search'),"<?php _e('Buscar inscripciones que coincidan con el texto indicado');?>");
