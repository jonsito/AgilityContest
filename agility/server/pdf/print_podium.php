...
class Print_Podium extends PrintCommon {
	...
	function writeTableHeader($mode) {
		...
		$this->Cell(12,6,_('Time'),0,0,'C',true);	// 1- Tiempo
		...
		if ($this->manga2!=null) {
			...
			$this->Cell(7,6,_('Ref'),0,0,'C',true);	// 2- Rehuses
			$this->Cell(12,6,_('Time'),0,0,'C',true);	// 2- Tiempo
	...
	function writeCell($idx,$row) {
		...
		$this->SetFont('Arial','B',10); // bold font
