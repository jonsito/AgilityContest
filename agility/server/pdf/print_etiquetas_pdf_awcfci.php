...
class Etiquetas_PDF extends PrintCommon {
	...
	function writeCell($idx,$row) {
        ...
        $rsce=getIconPath('RSCE','rsce.png');
        $fci=getIconPath('Intl3','fci.png');
		$this->Image($rsce,$this->getX(),$this->getY(),11);
		$this->Image($fci,$this->getX()+11,$this->getY()+7,9);
		...
		$this->Cell(45,5,$row['Nombre'].' - '.$row['NombreClub'],'',0,'L',false);
		$this->SetXY($left+161,$y5);
		$this->Cell(45,5,$row['NombreGuia'],'',0,'L',false);
		$this->SetXY($left+161,$y9);
		$this->Cell(45,5,'Tamás Tráj - J. L. García','',0,'L',false);
	}
