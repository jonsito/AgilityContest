...
if (!call_user_func_array('class_exists', $__tmp)) {
    ...
    define('FPDM_CACHE',__DIR__.'/../../../../logs/'); //cache directory for fdf temporary files needed by pdftk.
...
//					$this->Error("set_field_value failed as invalid valuetype $type for object $object_id");
		...
		**/
		...
		//--------------------------------------------
			...
			if($this->is_text_stream($stream['content'])) {
				$OldLen=$stream["length"]["value"];
				$lMin=$stream["start"];
				$lMax=$stream["end"];
				
				$stream_content=$this->_set_text_value($stream['content'],$value);
				...
				for($l=$lMin;$l<=$lMax;$l++) {
					...
					if($l==$lMin) {
						...
						// JAMC
                        // Seems that change_stream_value() is not used anymore
                        // next line gives an "unresolved error for $obj variable.
                        // so ask autor to fix
