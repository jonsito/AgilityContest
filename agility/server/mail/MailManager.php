...
class MailManager {
    ...
    public function sendInscriptions() {
        ...
        $htmlmsg .= "<p>CopyRight &copy; 2013-2021 by Juan Antonio Martinez &lt; jonsito at gmail dot com &gt;</p>";
        ...
        foreach ($pbdata['Jornadas'] as $j) { //buscamos la nuestra en la lista de jornadas
            ...
            foreach($j['Series'] as $s) {
                ...
                switch(intval($s['Tipo1'])) {
                    ...
                    case 1: case 2: // pre-agility
                        if ($this->myData['SendPreAgility']==0) break;
                        // si la jornada no tiene preAgility, no lo intentes parsear
                        if ( ($this->jornadaObj->PreAgility==0) && ($this->jornadaObj->PreAgility2==0)) break;
                        // no break
                    default: // individual
            ...
            foreach ($j['Mangas'] as $m) {
                ...
                switch(intval($m['TipoManga'])) {
                    ...
                    case 8: case 13:
                        $pdf=new PrintResultadosByEquipos3(
                            $this->myData['Prueba'],$this->myData['Jornada'],$manga,$resobj,$m['Mode'],_('Partial Scores')
                        );
                        break;
                    case 9: case 14:
                        $pdf=new PrintResultadosByEquipos4(
                            $this->myData['Prueba'],$this->myData['Jornada'],$manga,$resobj,$m['Mode'],_('Partial Scores')
                        );
                        break;
                    case 1: case 2: // pre-agility
