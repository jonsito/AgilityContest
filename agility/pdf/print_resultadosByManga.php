...
class PDF extends FPDF {
	...
	protected $cellHeader
					=array('Puesto','Dorsal','Lic.','Nombre.','Guía','Club','Cat/Grad','Falt.','Reh.','Toc.','Tiempo','Vel.','Penal','Calificacion');
	protected $pos	=array(  10,      10,     10,     12,     29,    31,     12,        7,      7,    7,       10,     7,    12,      25 );
	protected $align=array(  'L',     'R',    'R',    'R',    'R',   'R',    'C',       'C',   'C',   'C',     'R',    'R',  'R',     'L');
	...
	function __construct($prueba,$jornada,$manga,$resultados) {
		...
		if ( ($manga===null) || ($resultados===null) ) {
			$this->errormsg="printResultadosByManga: either manga or resultados data are invalid";
	...
	function composeTable() {
		...
		foreach($this->resultados['rows'] as $row) {
			...
			$veloc= ($row['Penalizacion']>=100)?"-":number_format($row['Velocidad'],1);
			$tiempo= ($row['Penalizacion']>=100)?"-":number_format($row['Tiempo'],2);
			...
			$this->Cell($this->pos[3],7,$row['Nombre'],			'LR',	0,		$this->align[3],	$fill);
...
$pdf = new PDF($prueba,$jornada,$manga,$resultados);
$pdf->AliasNbPages();
$pdf->composeTable();
