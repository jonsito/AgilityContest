...
class InscripcionPrinter extends DBObject {
	...
	function queryDB($prueba) {
		...
		while($row = $rs->fetch_array()){
			// $this->myLogger->trace("Examine dorsal ".$row['Dorsal']);
...
function doPrint($result) {
	...
	// OK now header is done: dump result data
	//initialize counter
	$i = 0;
...
try {
	...
	$ip =new InscripcionPrinter("printInscritorByPrueba");
	$result=$ip->queryDB(http_request("Prueba","i",0));
	if (!$result) throw new Exception($ip->errormsg);
	// and send data
	doPrint($result);
} catch (Exception $e) {
